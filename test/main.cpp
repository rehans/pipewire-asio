...
unsigned long current_time_in_milliseconds() {
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts); // Use CLOCK_MONOTONIC for monotonic time
...
#define TEST_RUN_TIME  1000000.0		// run for 20 seconds
	...
	for (int i = 0; i < asioDriverInfo.inputBuffers + asioDriverInfo.outputBuffers; i++)
	{
		if (asioDriverInfo.bufferInfos[i].isInput == false)
		{
			...
			case ASIOSTFloat32LSB:		// IEEE 754 32 bit float, as found on Intel x86 architecture
            {
                ...
                //break;
                const auto num_outs = asioDriverInfo.outputBuffers;
                void* inBuf = asioDriverInfo.bufferInfos[i - num_outs].buffers[index];
                void* outBbuf = asioDriverInfo.bufferInfos[i].buffers[index];
                memcpy (outBbuf, inBuf, buffSize * 4);
				break;
            }
